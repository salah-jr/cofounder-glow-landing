...
// Enhanced to support bidirectional resizing with improved responsiveness - faster resize
const ResizableHandle = ({
  ...
  return (
    <ResizablePrimitive.PanelResizeHandle
      className={cn(
        "relative flex w-px items-center justify-center cursor-col-resize transition-opacity duration-200",
      ...
      // Maximum responsiveness settings for fluid resize experience - faster updates
      tagName="div"
      data-superfluid="true"
      data-high-precision="true"
      style={{ 
        // Enhanced hit area and performance optimizations - faster response
        touchAction: "none",
        userSelect: "none",
    ...
    >
      {withHandle && (
        <div className="z-10 flex h-9 w-2 items-center justify-center rounded-full bg-white/10 backdrop-blur-sm transition-colors duration-200">
          <GripVertical className="h-3 w-3 text-white/50 transition-colors" />
        </div>
      )}
    </ResizablePrimitive.PanelResizeHandle>
  )
